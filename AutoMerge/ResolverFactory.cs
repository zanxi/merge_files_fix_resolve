...
namespace AutoMerge
{
        ...
        where T : IFileRelatedConflictResolver
    {
        ...
        {
            Type intf = typeof(T);
            foreach (Type type in typeof(FileResolverFactory).Assembly.GetTypes()) // пробегаем типы фрагментов, заданного шаблоном
            {
                if (type == null)
                    continue;
                ...
                var instance = (T)Activator.CreateInstance(type).NotNull(); // создание объекта из метаданных
